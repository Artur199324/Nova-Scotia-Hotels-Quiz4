    ...
    var body: some View {
        GeometryReader { geometry in
            VStack{
                ...
                ScrollView{
                    if  numberQwer == questions.questtion4.count-1 {
                        ...
                        // Вопрос
                        Image("\(questions.img4[numberQwer])")
                            .padding(.top, 10)
                        Text("\(questions.questtion4[numberQwer])")
                            .foregroundColor(.white)
                            .font(.title)
                            .multilineTextAlignment(.center)
                            .padding(30)
    ...
    func buttonWithDelay(index: Int, one: Binding<Bool>, two: Binding<Bool>, three: Binding<Bool>) -> some View {
        Button(action: {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                if questions.wrong4[numberQwer] == index + 1 {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                    ...
                    if numberQwer < questions.wrong4.count - 1 {
                        numberQwer += 1
                    }
                }
            }
        }, label: {
            Text("\(questions.answer4[numberQwer][index])")
                .foregroundColor(.white)
                .font(.title3)
