    ...
    var body: some View {
        GeometryReader { geometry in
            VStack{
                ...
                ScrollView{
                    if  numberQwer == questions.questtion2.count-1 {
                        ...
                        // Вопрос
                        Image("\(questions.img2[numberQwer])")
                            .padding(.top, 10)
                        Text("\(questions.questtion2[numberQwer])")
                            .foregroundColor(.white)
                            .font(.title)
                            .multilineTextAlignment(.center)
                            .padding(30)
    ...
    func buttonWithDelay(index: Int, one: Binding<Bool>, two: Binding<Bool>, three: Binding<Bool>) -> some View {
        Button(action: {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                if questions.wrong2[numberQwer] == index + 1 {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                    ...
                    if numberQwer < questions.wrong2.count - 1 {
                        numberQwer += 1
                    }
                }
            }
        }, label: {
            Text("\(questions.answer2[numberQwer][index])")
                .foregroundColor(.white)
                .font(.title3)
