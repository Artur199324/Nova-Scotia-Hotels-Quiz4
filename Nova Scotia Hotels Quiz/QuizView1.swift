    ...
    var body: some View {
        GeometryReader { geometry in
            VStack{
                ...
                ScrollView{
                    if  numberQwer == questions.questtion1.count-1 {
                        ...
                        // Вопрос
                        Image("\(questions.img1[numberQwer])")
                            .padding(.top, 10)
                        Text("\(questions.questtion1[numberQwer])")
                            .foregroundColor(.white)
                            .font(.title)
                            .multilineTextAlignment(.center)
                            .padding(30)
    ...
    func buttonWithDelay(index: Int, one: Binding<Bool>, two: Binding<Bool>, three: Binding<Bool>) -> some View {
        Button(action: {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                if questions.wrong1[numberQwer] == index + 1 {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                    ...
                    if numberQwer < questions.wrong1.count - 1 {
                        numberQwer += 1
                    }
                }
            }
        }, label: {
            Text("\(questions.answer1[numberQwer][index])")
                .foregroundColor(.white)
                .font(.title3)
